 ...
 *
 */
class CJT_Form_Journaltracking_Export extends CRM_Admin_Form
{
    ...
    function preProcess()
    {
        ...
        $url = CRM_Utils_System::url('civicrm/journaltracking', 'reset=1' . $id_string);
    ...
    public function buildQuickForm()
    {
        ...
        $this->addRule('code', ts('Code already exists in Database.'), 'objectExists', array('CJT_DAO_JournalTag', $this->_id, 'code'));
    ...
    /*
      ...
      $config = CJT_Core_Config::singleton();
      require_once( str_replace('_', DIRECTORY_SEPARATOR, $config->userHookClass) . '.php' );
      $null = & CJT_Core_DAO::$_nullObject;
      return
      eval('return ' .
    ...
    public function postProcess()
    {
        ...
        require_once 'journaltracking.php';
        ...
        $csvFullFilename = CRM_Utils_File::makeFileName('CiviReport.csv');
        ...
        $count = CJT_BAO_JournalTag::createActivities(JournalTrackingVal::ACTIVITY_CODE_PREFIX . $params['code'], $all_sql);

        // Record the journal batch
        $params['count'] = $count;
        $this->_recordJournalBatch($params);

        CRM_Core_Session::setStatus(ts('The journal distribution \'%1\' has been saved.', array(1 => $params['code'] ? $params['code'] : '')));
    ...
    {
        return ts('export for journal batch');
    }
    ...
    {
        // Record the journal batch
        $journal_batch = new CJT_DAO_JournalTag();
        $journal_batch->code = $params['code'];
        $journal_batch->activity_code = JournalTrackingVal::ACTIVITY_CODE_PREFIX . $params['code'];
        ...
        if ($result->_lastError) {
            CRM_Core_Session::setStatus(ts('There has been an error reorted in saving the journal batch. Please check that the journal batch has been recorded and watchdog log'));
        }
    }
